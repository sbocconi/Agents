...
startDocker ${cont_name} ${agent} ${mac_path} "${map_dirs}" ${SCRIPT_DIR}/GoDockerfile
...
if [ -f go.mod ]
then
  docker exec ${cont_name} bash -c "cd ${mac_path} && go mod download" || { echo 'go mod download failed'; exit 1; };
  docker exec ${cont_name} bash -c "cd ${mac_path} && go build -o main ." || { echo 'go build failed'; exit 1; };
fi

# Launch the Agent
echo "Container is UP. Launching ${agent} Agent..."
docker exec -it ${cont_name} bash -c "echo \"export IS_SANDBOX=${IS_SANDBOX}\" >> /root/.bashrc; source /root/.bashrc; ollama launch ${agent} --model ${model} -- ${add_options}"
